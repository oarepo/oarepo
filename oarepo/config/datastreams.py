...
def configure_datastreams(readers: dict[str, Any] | None = None, writers: dict[str, Any] | None = None,
    ...
    if readers:
        ...
        VOCABULARIES_DATASTREAM_READERS = merge_with_caller(objs)

    if writers:
        objs = {k: obj_or_import_string(v) for k, v in writers.items()}
        VOCABULARIES_DATASTREAM_WRITERS = merge_with_caller(objs)

    if transformers:
        objs = {k: obj_or_import_string(v) for k, v in transformers.items()}
        VOCABULARIES_DATASTREAM_TRANSFORMERS = merge_with_caller(objs)
